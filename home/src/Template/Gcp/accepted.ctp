		...
		<?= $this->Form->create($admin, [ 'context'=>[ 'validator'=>'admin' ], 'novalidate' => true ]) ?>
			...
			<?php if (count($applicants)==0) : ?>
				<?php if ( !empty($_GET['accepted']) ) : ?>
					<p><strong>There are currently no accepted applications matching that search</strong></p>
				<?php else : ?>
					<p><strong>There are currently no accepted applications</strong></p>
				<?php endif; ?>
			<?php else : ?>

				<!-- Submit -->
				<?php
					echo $this->Form->button(__('Reject Selected'), [ 'name'=>'reject', 'class'=>'btn button btn-primary' ]);
				...
				<p>
					You can reject individuals by selecting their checkbox and clicking on 'Reject Selected'.
				</p>
				<p>
				  <?php
				    $letters = str_split('abcdefghijklmnopqrstuvwxyz');
				    foreach ($letters as $l) {
				      echo $this->Html->link(strtoupper($l), ['?'=>['accepted'=>$l]]) . ' | ';
				    }
				    echo $this->Html->link('All', ['?'=>['accepted'=>'']]);
				  ?>
				</p>
