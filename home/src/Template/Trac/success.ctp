...
<?php
  $max = 37.5 * (!empty($survey->work_factor) ? $survey->work_factor : 1.0);
?>
  ...
  <div class="<?= $survey->group_colour ?>">
    ...
    <p>
      <strong>No entries or amendments will be accepted after collection closes on <?= $survey->group_finish_date ?>.</strong>
			...
			<?= $waf->postValueWithLabel($survey->department, 'Percentage of full time worked') ?>

			<hr class="line" />
			<?= $waf->postValueWithLabel($survey->gender, 'Gender', $survey->genderOptions()) ?>
			<?= $waf->postValueWithLabelIfNotZero($survey->pftw, 'Department/Faculty') ?>
      ...
      <p>&nbsp;</p>
			...
			<h4>Total</h4>
			<?= $survey->postContactPrepHoursPercentageWithLabel('abt', 'Teaching Total', $max, true) ?>
